...
public class HospitalMapsActivity extends FragmentActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        for (int i = 0; i < this.data_list.size(); i++) {
            ...
            markerArray.add(mMap.addMarker(new MarkerOptions().position(latLngList.get(i)).title(this.data_list.get(i).getNama()).icon(bitmapDescriptorFromVector(this, R.drawable.hospital))));
        }
        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(curLat,curLong),15.0f));
    }

    private BitmapDescriptor bitmapDescriptorFromVector(Context context, @DrawableRes int vectorDrawableResourceId) {
        Drawable background = ContextCompat.getDrawable(context, R.drawable.hospital);
